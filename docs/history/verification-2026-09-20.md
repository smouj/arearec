...
| Audio | WASAPI loopback smoke: 48 kHz, 2 channels, 32-bit, 3 chunks / 11,520 bytes; WGC + WASAPI + RecordingSession E2E (`22` decoded frames, AAC track); microphone fallback enumeration reaches active-device selection | Loopback/mixing/resampling/session/A-V mux PASS locally; no active microphone endpoint on this host, so microphone capture remains **NOT VERIFIED** |
| Self-contained x64 publish | `PUBLISH_PORTABLE.ps1` produced a 76,408,312-byte ZIP with `AreaRec.exe`, `hostfxr.dll`, `INSTALL_PORTABLE.ps1`, embedded AreaRec icon and matching SHA-256 (`660509a406ac75939671327d690ed1ac3278d678c7391127b4753cc7d7058983`); payload guards, icon extraction and packaged UI smoke pass | PASS locally |
...
The native product path is now the only supported runtime. MSIX packaging, tray
