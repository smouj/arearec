...
if (-not $exists) {
  gh repo create $Repo --public --source=. --remote=origin --push --description "Minimal local-first Windows region screen recorder powered by FFmpeg."
} else {
...
gh repo edit $Repo --enable-issues --disable-wiki --add-topic screen-recorder --add-topic ffmpeg --add-topic windows --add-topic python --add-topic offline --add-topic local-first --add-topic open-source
...
gh issue create --repo $Repo --title "test: Windows 10/11 end-to-end recording smoke test" --label "scope:v0.1,windows" --body "Validate region selection → 30/60 FPS recording → graceful stop → playable MP4 on Windows 10 and Windows 11. Record FFmpeg version and DPI scaling used."
...
gh issue create --repo $Repo --title "release: produce the first portable v0.1 package" --label "release,scope:v0.1" --body "Define a reproducible Windows portable package and document FFmpeg licensing/redistribution requirements before attaching binaries to a release."
